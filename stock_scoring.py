...
#TICKERS = ['UDR','VOYA','CBRE','ROP','JLL','CNI','VIVO','DGX','XENT','ABC','ZI','BCS']
TICKERS = ['COP','VOYA']

#TICKERS = ['ALLK', 'ATRI', 'CAMT', 'CARA', 'CCRN', 'CKPT', 'DSPG', 'ENOB', 'FULC', 'GALT', 'GRTS', 'GTS', 'IHC', 'IKNX', 'JMP', 'LQDA', 'LXRX', 'OBSV', 'PIRS', 'PRTA', 'PTMN', 'RMNI', 'VCYT', 'XLNX', 'ZS']
#TICKERS = ["KEYS","LULU"]#["DSPG","FULC"]#,"TX","NKE"]
...
def filter_percent_increase(df, region_indexes, min_percent_increase_multiplier, min_percent_increase_lim):
    ...
    df['natr'] = df['atr'].rolling(20).mean()
    natr_mean = df['natr'].iloc[-1]
...
def htf_score_calc(df, region_indexes):
    ...
    for rind in region_indexes:
        ...
        flag_max_check_length = min(flag_max_check_length, 8)
        ...
        for ind_flag in range(pole_start_ind+5, flag_start_ind+flag_max_check_length):
            ...
            if co_2days_tightness_flag  and tightness_flag_oc: # HTF tightness condition
                ...
                # print(df.Close[ind_flag-3:ind_flag])
                # print(df.Close[ind_flag-3:ind_flag].mean())
                # print(df.Close[ind_flag])
                # print(htf_weighted_slope_score)
                ...
                natr_score = linear_scorer(natr_mean, 10, 1, 0.03, 0.015)
                ...
                htf_score = ((pole_score_sat**0.5)*ftpr_score*SMA_cond_score*ave_cons_tightness_score*natr_score*htf_weighted_slope_score)**(1./5.5)
...
if __name__ == '__main__':
    for ticker in TICKERS:
        ...
        if not gl_df_htf.empty: FINAL_RESULT_FRAME = FINAL_RESULT_FRAME.append(gl_df_htf.tail()) # populating the final result frame for every ticker
